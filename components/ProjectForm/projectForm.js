  ...
  const handleSubmit = (event) => {
    ...
    if (postToUpdate) {
      if (uploadFile) {
        ...
        editPost(postToUpdate.id, { ...dataRegister }).then((post) => {
          setPostUpdate(post);
        });
      }
      closeModal();
      ...
      uploadCloudinary(formData)
        .then((response) => {
          registerProject({ ...dataRegister, image: response.secure_url })
            .then((response) => {
        ...
        >
          <div className="space-y-1 text-center">
            ...
            <div className="flex flex-col gap-3 text-sm text-gray-600">
              <label
                htmlFor="image"
                className="relative flex flex-col justify-center items-center w-full cursor-pointer rounded-md bg-white font-medium text-primary focus-within:outline-none focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2 hover:text-primary"
              >
